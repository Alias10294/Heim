...
{
/**
 * @brief A glimpse of the current state of the components of given type of the
 * world.
 * @tparam Comps The types of the components of the glimpse.
 */
template<typename... Comps>
class basic_glimpse
{
public:
  using compositions_tuple =
      std::tuple<composition<std::remove_cvref_t<Comps>>...>;

  class iterator
  {
  public:
    explicit constexpr iterator(
        compositions_tuple& compositions,
        const std::size_t   ref_index = 0ULL)
  ...
  private:
    ...
    /**
     * @brief Advances to the next entity that has all the components of the
     * glimpse.
     */
    constexpr void advance_to_valid()
    {
      auto valid = [&]()
      {
        return std::apply(
            [&](auto&... compositions)
            {
              return (compositions.contains(
                  std::get<0>(compositions_).composed(ref_index_)) && ...);
            },
            compositions_);
      };
  ...
  explicit constexpr basic_glimpse(
      composition<std::remove_cvref_t<Comps>>&... compositions) noexcept
    : compositions_{ compositions... }
  { }
